...
static int portfs_iterate_shared(struct file *filp, struct dir_context *ctx)
{
    ...
    for (; ctx->pos - 2 < num_entries; ctx->pos++)
    {
        ...
        struct inode *inode = portfs_get_inode_by_number(inode->i_sb, d_entry->inode_number);
        if (!inode || IS_ERR(inode) || !inode->i_private)
            continue;
        ...
        if (!dir_emit(ctx, d_entry->name, strlen(d_entry->name),
                      d_entry->inode_number, mode_to_dtype(inode->i_mode)))
            return -ENOMEM;
    }
